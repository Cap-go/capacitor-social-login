...
| **`clientSecret`** | <code>string</code>             | TikTok client secret. Provide this if you want the plugin to exchange the authorization code for tokens client-side. If omitted, only PKCE public-client token exchange will succeed. |                                  |
| **`scopes`**       | <code>string[]</code>           | Scopes to request during login.                                                                                                                                                       | <code>['user.info.basic']</code> |
