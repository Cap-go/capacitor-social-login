...
extension AppleProviderError: LocalizedError {
    var errorDescription: String? {
        ...
        case .alamofireNotAvailable:
            return NSLocalizedString("Apple redirectUrl / backend token exchange requires Alamofire. Add Alamofire to your Podfile or Package.swift, or remove redirectUrl to use basic Sign in with Apple.", comment: "")
        }
    }
}

// Basic Sign in with Apple uses AuthenticationServices only.
// Alamofire is required only for redirectUrl / backend token-exchange flows.
