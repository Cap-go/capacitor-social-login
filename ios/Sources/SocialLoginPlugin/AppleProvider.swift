...
extension AppleProviderError: LocalizedError {
    var errorDescription: String? {
        ...
        case .alamofireNotAvailable:
            return NSLocalizedString("Advanced Apple Sign-In features (redirectUrl, backend token exchange) require Alamofire dependency. Either add Alamofire to your Podfile/Package.swift, or remove the redirectUrl configuration to use basic Sign in with Apple.", comment: "")
        }
    }
}

// AppleProvider class - works with or without Alamofire
// Basic Sign in with Apple works without Alamofire
// Advanced features (redirectUrl, backend flows) require Alamofire
class AppleProvider: NSObject, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    func initialize(redirectUrl: String? = nil, useProperTokenExchange: Bool = false) {
        if let redirectUrl = redirectUrl {
            ...
            // Check if Alamofire is available when redirectUrl is set
            #if !canImport(Alamofire)
            print("WARNING: redirectUrl is set but Alamofire is not available. Backend flows will fail. Please add Alamofire dependency or remove redirectUrl configuration.")
            #endif
        }
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            if !self.redirectUrl.isEmpty {
                ...
                #else
                // Alamofire is required for backend redirectUrl flows
                self.completion?(.failure(AppleProviderError.alamofireNotAvailable))
                #endif
            } else {
                do {
    ...
    // MARK: - Alamofire-dependent methods (backend flows)
    #if canImport(Alamofire)
