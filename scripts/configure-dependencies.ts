 ...
 */
function log(message: string, emoji: string = '', color: string = ''): void {
...
/**
 * Get emoji and color for provider status
 */
function getProviderStatus(provider: string, enabled: boolean, dependencyType?: string | boolean): { emoji: string; color: string } {
  if (dependencyType === 'compileOnly') {
    return { emoji: '⚡', color: colors.yellow };
  }
  if (dependencyType === 'implementation') {
    return { emoji: '✔', color: colors.green };
  }
  // Default to enabled (implementation)
  return { emoji: '✔', color: colors.green };
}
...
function configureIOS(providerConfig: ProviderConfig): void {
  ...
  try {
    ...
    const replacements: Array<{ old: RegExp; new: string }> = [
      {
