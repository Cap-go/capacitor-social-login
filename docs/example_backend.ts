...
app.post("/login/callback", async (c) => {
  ...
  if (userStr) {
    ...
    if (
      ...
      console.log(
        `no jwt?? JWT: ${parsedJwt} Data: ${JSON.stringify(appleData)}`,
      );
      return c.redirect("?success=false");
    }
  ...
  return c.redirect(
    `?success=true&code=${body.get("code")}&client_secret=${clientSecret}`,
  );
});
...
app.get("/userdata", async (c) => {
  ...
  try {
    ...
    return c.json(
      { error: "Cannot convert apple key", more_detailed: JSON.stringify(e) },
      500,
    );
  }
