...
public class FunctionResult<SUCCESS, ERROR> {
  ...
  public <NEW_SUCCESS> FunctionResult<NEW_SUCCESS, ERROR> disregardSuccess() {
    if (!this.isError()) {
      throw new RuntimeException(
        "Cannot disregard success, function is successful"
      );
    }
  ...
  public static <SUCCESS, ERROR> FunctionResult<SUCCESS, ERROR> success(
    SUCCESS success
  ) {
    return new FunctionResult<>(success, null);
  }

  public static <SUCCESS, ERROR> FunctionResult<SUCCESS, ERROR> error(
    ERROR error
  ) {
